...
export const SignupFunction = async (e, name, email, password, confirmPassword, setResponse) => {
    e.preventDefault();
    try {
        if (password !== confirmPassword) {
            return setResponse('Password and confirm password do not match');
        }

        const response = await axios.post(`${base}/sign-up`, {
            fullName: name,
            email,
            password
        });
